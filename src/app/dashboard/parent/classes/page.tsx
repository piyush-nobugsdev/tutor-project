import { getParentClasses } from '@/app/actions/applications'
import ParentClassCard from '@/components/classes/ParentClassCard'

export default async function ParentClassesPage() {
  const { data: classes, error } = await getParentClasses()
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-1">
        ...
        <p className="text-sm text-gray-500">
          Active tutoring engagements for your children
        </p>
      </div>
      ...
      {error ? (
        <div className="rounded-xl border border-red-100 bg-red-50 px-5 py-4">
          <p className="text-sm text-red-600">
            Could not load classes. Please refresh the page.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50/50 py-16 px-8 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1 max-w-xs mx-auto">
            Accept a tutor application from your jobs page to start a class.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {classes.map((cls) => (
            <ParentClassCard key={cls.jobId} cls={cls} />
          ))}
        </div>
      )}
    </div>
